...
from .randomization import LandingRpsDomainRandomizationCfg, LandingRpsDomainRandomizationState
...
class PlatformMotionStageCfg:
    ...
    max_roll_abs_rad: float | None = None
    max_pitch_abs_rad: float | None = None
...
def add_platform_top_decal(
    ...
    if not texture_file.is_file():
        print(
            "[WARN][landing_rps] Platform texture PNG not found at "
...
class SampleLandingRpsDomainRandomization(ManagerTermBase):
    """Sample per-environment randomization flags/parameters and apply physics-side mass noise."""

    def __init__(self, cfg: EventTermCfg, env: "ManagerBasedEnv"):
        super().__init__(cfg, env)
        self.rand_cfg: LandingRpsDomainRandomizationCfg = cfg.params["rand_cfg"]
        self.mass_asset_cfg: SceneEntityCfg = cfg.params["mass_asset_cfg"]
        self.asset: Articulation = env.scene[self.mass_asset_cfg.name]
        self.runtime_state = LandingRpsDomainRandomizationState(self.rand_cfg, self.num_envs, self.device)
        # landing_rps-specific runtime slot.
        setattr(env, "_landing_rps_domain_randomization", self.runtime_state)
    ...
    def __call__(
        ...
        rand_cfg: LandingRpsDomainRandomizationCfg | None = None,
...
SampleVanillaDomainRandomization = SampleLandingRpsDomainRandomization
...
class MultiSinePlatformMotion(ManagerTermBase):
    ...
    def _evaluate_orientation(
        ...
        if self.stage_cfg.max_roll_abs_rad is not None:
            roll = torch.clamp(roll, min=-float(self.stage_cfg.max_roll_abs_rad), max=float(self.stage_cfg.max_roll_abs_rad))
        if self.stage_cfg.max_pitch_abs_rad is not None:
            pitch = torch.clamp(
                pitch,
                min=-float(self.stage_cfg.max_pitch_abs_rad),
                max=float(self.stage_cfg.max_pitch_abs_rad),
            )
        ...
        prev_roll = prev_values[:, 3]
        prev_pitch = prev_values[:, 4]
        next_roll = next_values[:, 3]
        next_pitch = next_values[:, 4]
        if self.stage_cfg.max_roll_abs_rad is not None:
            roll_cap = float(self.stage_cfg.max_roll_abs_rad)
            prev_roll = torch.clamp(prev_roll, min=-roll_cap, max=roll_cap)
            next_roll = torch.clamp(next_roll, min=-roll_cap, max=roll_cap)
        if self.stage_cfg.max_pitch_abs_rad is not None:
            pitch_cap = float(self.stage_cfg.max_pitch_abs_rad)
            prev_pitch = torch.clamp(prev_pitch, min=-pitch_cap, max=pitch_cap)
            next_pitch = torch.clamp(next_pitch, min=-pitch_cap, max=pitch_cap)
        quat_prev = math_utils.quat_mul(
            base_quat,
            math_utils.quat_from_euler_xyz(prev_roll, prev_pitch, prev_values[:, 5]),
        )
        quat_next = math_utils.quat_mul(
            base_quat,
            math_utils.quat_from_euler_xyz(next_roll, next_pitch, next_values[:, 5]),
        )
