...
class HeaveLandingRewardWeightsCfg:
    ...
    # mdp.heave_cbf_h0_signed: reward positive h0 and penalize negative h0.
    cbf_h0: float = 0.1
...
class HeaveLandingCbfCfg:
    ...
    landing_velocity_mps: float = -0.4
    # Conservative relative vertical deceleration capability.
    a_rel_mps2: float = 0.7
    # Smoothing epsilon used for downward relative velocity.
    eps: float = 1.0e-4
    # Reward margin: penalty starts when h0 is below this positive buffer.
    h0_margin_m: float = 0.2
    # If True, terminate the episode when h0 < 0.
    terminate_on_negative: bool = False
...
class HeaveLandingPostInitCfg:
    ...
    def apply(self, env_cfg: HeaveLandingEnvCfg) -> None:
        ...
        env_cfg.rewards.cbf_h0.weight = self.reward_weights.cbf_h0
        ...
        cbf_params = {
            "d_min_m": float(self.cbf.d_min_m),
            "landing_velocity_mps": float(self.cbf.landing_velocity_mps),
            "a_rel_mps2": float(self.cbf.a_rel_mps2),
            "eps": float(self.cbf.eps),
        }
        env_cfg.rewards.cbf_h0.params.update(cbf_params)
        env_cfg.terminations.cbf_h0_negative.params.update(cbf_params)
        env_cfg.terminations.cbf_h0_negative.params["enabled"] = bool(self.cbf.terminate_on_negative)
        env_cfg.curriculum.cbf_h0_metrics.params.update(cbf_params)
        env_cfg.curriculum.cbf_h0_metrics.params["margin_m"] = float(self.cbf.h0_margin_m)
        env_cfg.curriculum.cbf_h0_metrics.params["gamma"] = float(self.cbf.gamma)
        env_cfg.observations.critic.cbf_h0_features.params.update(cbf_params)
