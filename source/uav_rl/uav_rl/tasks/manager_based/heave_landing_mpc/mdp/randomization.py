...
@configclass
class HeaveLandingDomainRandomizationCfg:
    """Domain-randomization switches and sampling ranges for the heave_landing_mpc UAV task."""
...
class HeaveLandingDomainRandomizationState:
    """Per-environment runtime state sampled from :class:`HeaveLandingDomainRandomizationCfg`."""

    def __init__(self, cfg: HeaveLandingDomainRandomizationCfg, num_envs: int, device: torch.device | str):
        self.cfg = cfg
...
def get_domain_randomization_state(env) -> HeaveLandingDomainRandomizationState | None:
    state = getattr(env, "_heave_landing_domain_randomization", None)
    if state is not None:
        return state
    # Backward compatibility for runs created before the heave_landing attribute rename.
...
# Backward-compatible aliases for older configs/checkpoints.
VanillaDomainRandomizationCfg = HeaveLandingDomainRandomizationCfg
VanillaDomainRandomizationState = HeaveLandingDomainRandomizationState
