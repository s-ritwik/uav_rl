...
def heave_cbf_h0_signed(
    ...
    """Return signed h0: positive reward signal when safe, negative when unsafe."""
...
def heave_cbf_h0_metrics(
    ...
    """Return reset-time CBF safety metrics for the environments being reset."""
    ...
    if env_ids_tensor.numel() == 0:
        return {
            "cbf_h0_negative_rate": 0.0,
            "cbf_h0_margin_violation_rate": 0.0,
            "cbf_h0_mean_m": 0.0,
            "cbf_h0_min_m": 0.0,
            "cbf_dstop_mean_m": 0.0,
            "cbf_vr_neg_mean_mps": 0.0,
            "cbf_gamma": float(gamma),
        }
    ...
    h0 = h0[env_ids_tensor]
    dstop = dstop[env_ids_tensor]
    vr_neg = vr_neg[env_ids_tensor]
    return {
        "cbf_h0_negative_rate": float((h0 < 0.0).float().mean().item()),
        "cbf_h0_margin_violation_rate": float((h0 < float(margin_m)).float().mean().item()),
        "cbf_h0_mean_m": float(h0.mean().item()),
        "cbf_h0_min_m": float(h0.min().item()),
        "cbf_dstop_mean_m": float(dstop.mean().item()),
        "cbf_vr_neg_mean_mps": float(vr_neg.mean().item()),
        "cbf_gamma": float(gamma),
    }
